...
import { toggleWrappersAndHooks } from "./_utils";
...
class ActionableTool extends ModuleTool<ToolSettings> {
    ...
    _configurate(): void {
        ...
        toggleWrappersAndHooks(this.#actionHooksAndWrappers, actionEnabled);

        this.#renderCharacterSheetPF2eHook.toggle(actionEnabled);

        renderItemSheets(["FeatSheetPF2e", "AbilitySheetPF2e"]);
        renderCharacterSheets();
    }

    getItemMacro(item: ItemPF2e): Maybe<MacroPF2e | CompendiumIndexData> {
        if (
            item.isOfType("spell") ||
            (item.isOfType("feat", "action") && !item.system.selfEffect && !item.crafting)
        ) {
            const uuid = this.getFlag<string>(item, "macro");
            const macro = uuid ? fromUuidSync<Macro>(uuid) : undefined;
    ...
    #updateCharacterActions(
        ...
        for (const { id: itemId, img: actionImg } of actions) {
            const el = htmlQuery(panel, `.actions-list .action[data-item-id="${itemId}"]`);
            const item = actor.items.get(itemId);
            if (!el || !item?.isOfType("action", "feat") || !this.isValidAction(item)) continue;

            const macro = this.getItemMacro(item);
            if (!macro) continue;
    ...
    async #onRenderActionSheetPF2e(sheet: AbilitySheetPF2e | FeatSheetPF2e, $html: JQuery) {
        ...
        addListenerAll(newDropzone, "[data-action]", (el) => {
            switch (el.dataset.action as "open-link-sheet" | "delete-link") {
                ...
                case "delete-link": {
                    return sheet.isEditable && this.unsetFlag(item, "macro");
                }
            }
        });
    }
