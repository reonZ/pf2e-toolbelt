...
class HeroActionsTool extends ModuleTool<HeroActionsSettings> {
    ...
    #drawHeroActionsEmitable = createEmitable(this.path("draw"), this.#drawHeroActions.bind(this));
    ...
    get api(): toolbelt.Api["heroActions"] {
        return {
    ...
    isValidActor(actor: unknown): actor is CharacterPF2e {
        ...
        this.localize.warning("error.onlyCharacter");
        return false;
    }
    ...
    async getHeroActionDetails(uuid: string) {
        ...
        if (!entry) {
            this.localize.error("error.noDetails");
            return;
        }
    ...
    getHeroActionsTemplateData(actor: CharacterPF2e): toolbelt.heroActions.HeroActionsTemplateData | undefined {
    ...
    async removeHeroActions(): Promise<void> {
        if (!game.user.isGM) {
            this.localize.warning("error.notGM");
            return;
        }

        const onRender: fa.api.DialogV2RenderCallback = (_event, dialog) => {
        ...
        this.localize.info("removeActions.removed");
    }
    ...
    async giveHeroActions(actor: CharacterPF2e): Promise<void> {
        ...
        if (!table) {
            this.localize.error("table.notFound", true);
            return;
        }
        ...
        const result = await waitDialog<{
            ...
            content: this.templatePath("giveActions"),
            data: {
                actions: R.sortBy(R.filter(actionsList, R.isTruthy), R.prop("name")),
                isUnique,
            },
            i18n: this.path("giveActions"),
        ...
        await this.setFlag(actor, "actions", actions);
    ...
    async drawHeroActions(actor: CharacterPF2e): Promise<void> {
        ...
        if (nb <= 0) {
            this.localize.warning("error.notEnoughPoint");
            return;
        }
    ...
    async discardHeroActions(actor: CharacterPF2e, uuids: string[] | string) {
        if (!this.isValidActor(actor)) return;
    ...
    async tradeHeroAction(actor: CharacterPF2e): Promise<void> {
        ...
        if (!actions.length) {
            this.localize.warning("error.noAction");
            return;
        }
        ...
        if (!usesCount && heroPoints < actions.length) {
            this.localize.warning("error.mustDiscard", { nb: actions.length - heroPoints });
            return;
        }
        ...
        const others: TradeActor[] = game.actors
            .map((other): TradeActor | undefined => {
                if (!other.isOfType("character") || other === actor || !other.hasPlayerOwner) return;
        ...
        if (others.length === 0) {
            this.localize.warning("error.noOther");
            return;
        }
    ...
    async #exchangeActions({ origin, target }: TradeExchangeOptions) {
        ...
        await this.setFlag(originActor, "actions", originActions);
        await this.setFlag(targetActor, "actions", targetActions);
    ...
    async #expandActionDescription(_actor: CharacterPF2e, uuid: string, target: Maybe<HTMLElement>) {
    ...
    async #drawHeroActions({ actor, table }: DrawHeroActionsOptions, _userId: string) {
        if (!(actor instanceof Actor) || !actor.isOfType("character") || !(table instanceof RollTable)) return;

        const drawn: HeroAction[] = [];
        const count = this.settings.count;
        const actions = this.getHeroActions(actor);
        const nb = count || getMythicOrHeroPoints(actor).value - actions.length;
        ...
        for (let i = 0; i < nb; i++) {
            ...
            if (!draw) {
                this.localize.warning("table.noDraw", { name: actor.name });
                return;
            }
            ...
            if (!uuid) {
                this.localize.warning("table.drawError");
                return;
            }
    ...
    #createActionsMessage(
        ...
        { senderId = game.user.id, other, otherId }: { senderId?: string; other?: string; otherId?: string } = {},
    ) {
        ...
        const data: DeepPartial<ChatMessageSourcePF2e> = {
...
async function openDescriptionFromElement(el: HTMLElement) {
    ...
    if (entry instanceof JournalEntryPage) {
        entry.parent.sheet?.render(true, { pageId: entry.id } as any);
    } else {
...
type BaseSocketOption<T extends string = string, D extends Record<string, any> = Record<string, any>> = D & {
    type: T;
};
...
export { HeroActionsTool, openDescriptionFromElement };
