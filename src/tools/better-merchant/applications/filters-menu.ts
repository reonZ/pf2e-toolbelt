...
class FiltersMenu extends ModuleToolApplication<BetterMerchantTool> {
    ...
    constructor(tool: BetterMerchantTool, actor: LootPF2e, options?: DeepPartial<fa.ApplicationConfiguration>) {
        super(tool, options);
    ...
    async _prepareContext(_options: fa.ApplicationRenderOptions): Promise<FiltersMenuContext> {
    ...
    getDefaultFilter<T extends FilterType>(type: T): DefaultFilterTypes[T] | undefined {
        return this.tool.getDefaultFilter(this.actor, type);
    }

    getFilters<T extends FilterType>(type: T): FilterTypes[T][] {
        return this.tool.getFilters(this.actor, type);
    }

    setFilters<T extends FilterType>(type: T, filters: FilterTypes[T][]): Promise<LootPF2e> {
        return this.tool.setFilters(this.actor, type, filters);
    }
    ...
    protected _activateListeners(html: HTMLElement): void {
        ...
        addListenerAll(html, "input", "change", async (target) => {
            ...
            if (filterId === "default") {
                ...
                await this.tool.setFlag(this.actor, "default", type, key, value);
            } else {
                ...
                await this.setFilters(type, filters);
            }
    ...
    async #deleteFilter(type: FilterType, filterId: string) {
        const confirm = await confirmDialog(this.path("delete"));
        if (!confirm) return;
    ...
    async #editFilter(type: FilterType, filterId: string) {
        ...
        const callback = async () => {
            filter.filter = await this.#getEquipmentFilter();
            await this.setFilters(type, filters);
            this.render();
        };
    ...
    #addFilter(type: FilterType) {
        ...
        if (type === "service") {
            const data = ServiceFilter.schema.parse({});
            const filter = new ServiceFilter(data);
...
type FiltersMenuContext = fa.ApplicationRenderContext & {
