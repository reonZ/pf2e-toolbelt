...
import { createConsumableFromSpell, getSpellRankLabel } from "foundry-helpers/dist";
...
class BrowserPullMenu extends ModuleToolApplication<BetterMerchantTool> {
    ...
    async _prepareContext(_options: fa.ApplicationRenderOptions): Promise<BrowserPullContext> {
        const entries = R.map(this.#results, (data): ItemEntry => {
            const ranks: { value: string; label: string }[] = R.isNumber(data.rank)
    ...
    async #addToMerchant() {
        ...
        const confirm = await confirmDialog(this.path(), {
...
type BrowserPullContext = fa.ApplicationRenderContext & {
    entries: ItemEntry[];
};
