...
class RollTracker extends ModuleToolApplication<RollTrackerTool> {
    ...
    constructor(tool: RollTrackerTool, options: DeepPartial<fa.ApplicationConfiguration> = {}) {
        super(tool, options);
    ...
    get modes(): RollsMode[] {
        ...
        modes.push(
            ...
            createRollMode("attack-roll"),
            {
                ...
                entries: DEGREE_OF_SUCCESS_STRINGS.map((x) => {
                    return game.i18n.localize(`PF2E.Check.Result.Degree.Attack.${x}`);
                }),
                rolls: (rolls) => {
                ...
                values: (rolls) => {
                    return R.pipe(
                        R.range(0, 4) as ZeroToThree[],
                        R.map((i) => {
                            const outcome = degreeOfSuccessString(i);
                            return rolls.filter((roll) => roll.outcome === outcome).length;
                        }),
                    );
                },
            },
        ...
        return (this.#modes = modes.map(({ entries, rolls, type, values }) => {
            const labels = (this.localize.ifExist("mode", type, "bottom") ?? this.localize("bottom")).split("|");
    ...
    async _prepareContext(_options: fa.ApplicationRenderOptions): Promise<RollTrackerContext> {
        const { encounters, sessions } = this.#getTimedEventArrays();
    ...
    #getTimedEventArrays(): { encounters: TimedEventEntry[]; sessions: TimedEventEntry[] } {
        const [encounters, sessions] = R.pipe(
            [this.tool.encounters, this.tool.sessions],
            R.map((entries) => {
                return R.pipe(
                    entries,
                    R.entries(),
...
function formatTimedEventOption({ id, time }: TimedEventEntry): { value: string; label: string } {
...
type RollTrackerContext = fa.ApplicationRenderContext & {
    encounter: string;
    encounters: { value: string; label: string }[];
    inSession: boolean;
    isGM: boolean;
    isPaused: boolean;
    filter: FilterType;
