...
export class IdentifyTracker extends ModuleToolApplication<IdentifyTool> {
    ...
    constructor(item: Maybe<ItemPF2e>, tool: IdentifyTool, options?: DeepPartial<fa.ApplicationConfiguration>) {
        super(tool, options);
    ...
    async _prepareContext(_options: IdentifyRenderOptions): Promise<IdentifyContext> {
        ...
        const itemGroups = new MapOfArrays<PhysicalItemType, IdentifyGroupItem>();
        ...
        await Promise.all(
            allItems.map(async (item) => {
                ...
                const itemSlug = scrollWandSpell
                    ? (scrollWandSpell.slug ?? SYSTEM.sluggify(scrollWandSpell.name))
                    : (item.slug ?? SYSTEM.sluggify(data.name));
                const partialSlug =
                    isConsumable && !scrollWandSpell
                        ? SYSTEM.sluggify(data.name.replace(PARTIAL_SLUGH_REGEX, ""))
                        : undefined;
    ...
    for (const entry of actor.spellcasting.spellcastingFeatures) {
        ...
        for (const spell of entry.spells ?? []) {
            if (spell.rarity !== "common" || !tradition) {
                known.add(spell.slug ?? SYSTEM.sluggify(spell.name));
            }
        }
    }
...
type IdentifyContext = fa.ApplicationRenderContext & {
