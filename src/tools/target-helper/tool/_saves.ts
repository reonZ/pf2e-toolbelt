...
import { DEGREE_OF_SUCCESS_STRINGS, DegreeOfSuccess, extractNotes, RollNotePF2e } from "foundry-helpers/dist";
import {
    RerollType,
...
async function rollSaves(
    ...
    const extraRollOptions = targetHelper.extraRollOptions;
    ...
    const updates: Record<string, TargetSaveInstance> = {};

    const targetsRollsPromise = targets.map((target) => {
        if (targetHelper.targetSave(target.id)) return;
        ...
        return new Promise<void>((resolve) => {
            const callback: CheckRollCallback = async (roll, success, rollMessage) => {
                ...
                const modifiers = R.pipe(
                    rollMessage.flags[SYSTEM.id].modifiers ?? [],
                    R.filter((modifier) => !!modifier.enabled),
                    R.map((modifier): SaveInstanceModifier => {
                        return {
                            excluded: false,
                ...
                const rollData: TargetSaveInstance = {
                    ...
                    significantModifiers: window.pf2eMm?.getSignificantModifiersOfMessage(rollMessage) ?? [],
...
async function rerollSave(
    ...
    const rerolls = R.entries(REROLLS);
    ...
    } satisfies TargetSaveInstanceSource;
...
export { REROLLS, rerollSave, rollSaves };
